import { db, tenants, tenantMembers, products, quotes, apiKeys, eq, desc, sql } from '@aerolume/db';
...
export default async function AdminTenantDetailPage({ params }: PageProps) {
  ...
  const productList = await db.select({ id: products.id, name: products.name, sailType: products.sailType, active: products.active }).from(products).where(eq(products.tenantId, id));
  ...
  return (
    <div className="space-y-6">
      ...
      <TenantDetailClient tenant={tenant} members={members} products={productList} quotes={quoteList} apiKeys={keys} />
    </div>
  );
}
